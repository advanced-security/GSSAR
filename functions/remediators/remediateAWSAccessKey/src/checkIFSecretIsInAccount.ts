...
export const checkIFSecretIsInAccount = async (
  { secret }: InputFromStateMachine,
  UserName: string
): Promise<response> => {
  ...
  try {
    ...
    const { AccessKeyMetadata, IsTruncated, Marker } = await client.send(
      command
    ); // TODO: Paginate

    if (IsTruncated)
      console.log(
        "Ah man, there are more results to get, the marker is: ",
        Marker
      );
